...
if ( !class_exists( 'FjqGrid' ) ) {
	...
	class FjqGrid
	{
		...
		static function fplugin_uninstall()
		{
			...
			file_put_contents( dirname( __FILE__ ) . '/log.txt', $time . ' : delete=' . $plugin . "\n", FILE_APPEND | FILE_TEXT );		
			if ( $plugin == 'wp-fjqgrid/index.php' ) {
				// delete options on deactivate
				$options = get_option( 'wpf-jqgrid' );
				file_put_contents( dirname( __FILE__ ) . '/log.txt', $time . ' : wpf-jqgrid delete option=' . $options['do_uninstall'] . "\n", FILE_APPEND | FILE_TEXT );
				if ( $options['do_uninstall'] == "1" ) {
					//drop tables in list
					global $wpdb;
					foreach ( explode( ",", $options['do_drop'] ) as $table ) {
						$wpdb->query( "DROP TABLE IF EXISTS `$table`" );
						file_put_contents( dirname( __FILE__ ) . '/log.txt', $time . ' : wpf-jqgrid deleted table ' . $table . "\n", FILE_APPEND | FILE_TEXT );
					}
					//remove option entry
					delete_option( 'wpf-jqgrid' );
					delete_site_option( 'wpf-jqgrid' );
				}
			}
		}
		...
		static function fplugin_deactivate()
		{
			...
			$time = date( 'd-m-Y H:i:s ' );
			file_put_contents( dirname( __FILE__ ) . '/log.txt', $time . ' : wpf-jqgrid DEACTIVATED' . "\n", FILE_APPEND | FILE_TEXT );
		}
		...
		public function fplugin_load_scripts()
		{
			...
			//if ( $this->wpf_loglevel == 3 )//DEBUG only
				wp_register_script( 'jqg_code', $this->wpf_path.'jqGrid/js/jquery.jqGrid1.src.js' );
			//else
		...
		public function add_foption_init()
		{
		...
		public function foptions_validate( $input )
		{
			...
			if ( isset( $input['do_createtable'] ) ) {
				unset( $input['do_createtable'] );
			}
			if ( isset( $input['createtable'] ) ) {
				unset( $input['createtable'] );
			}
			return $input;
		}
...
Verified::int(1) DEFAULT NULL";
			...
			<div class="wrap">
				...
				<form method="post" action="options.php">
					...
					<table class="form-table">
						<tr style="vertical-align: top"><th scope="row"><?php _e( 'active', $this->wpf_code ); ?></th>
							<td><input type="checkbox" name="<?php echo $this->wpf_code; ?>[active]" value="1" <?php checked( '1', $options['active'] ); ?> /></td>
						</tr>
						<tr style="vertical-align: top"><th scope="row"><?php _e( 'log level', $this->wpf_code ); ?></th>
						...
						<tr style="vertical-align: top"><th scope="row"><?php _e( 'role required to edit tables', $this->wpf_code ); ?></th>
							<td>
						...
						<tr style="vertical-align: top"><th scope="row"><?php _e( 'allowed tables (comma separated)', $this->wpf_code ); ?></th>
		...
		{
		    //FIXME @@@SC 2021030 do not use the removed get_magic_quotes_gpc();
